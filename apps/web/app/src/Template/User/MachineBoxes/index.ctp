    ...
    ?>
        ...
        <tr
          ...
          <td>
            ...
            <?= h($data->machine_content->name); ?>
            [<?= $data->machine_content->serial_no; ?>]
          ...
          <td>
            <?php if ($data->machine_content_id && $data->content): ?>
            <span class="btn_area">
              <a href="<?= $this->Url->build(['controller' => 'contents', 'action' => 'edit', $data->content_id, '?' => ['mode' => 'machine']]); ?>"
                class="btn btn-primary btn-sm"><i class="fas fa-edit"></i> コンテンツの編集</a>
