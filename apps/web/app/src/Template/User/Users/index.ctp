...
<?= $this->Form->end(); ?>
      ...
      <div class="box">
        <h3 class="box__caption--count"><span>登録一覧</span><span class="count"><?php echo $numrows; ?>件の登録</span></h3>
        <div class="btn_area" style="margin-top:10px;">
          <a href="<?= $this->Url->build(array('action' => 'edit', '?' => [])); ?>" class="btn_confirm btn_post">新規登録</a>
          <a href="<?= $this->Url->build(array('action' => 'import', '?' => [])); ?>" class="btn_confirm btn_post">CSVファイル一括登録</a>
        </div>
...
?>
            ...
            <tr class="<?= $status ? "visible" : "unvisible" ?>" id="content-<?= $data->id ?>">
              ...
              <td style="text-align: center;">
                <div class="<?= $status ? "visi" : "unvisi" ?>"><?= $this->Html->link(($status? "有効" : "無効" ), array('action' => 'enable', $data->id, '?' => $query) )?></div>
              </td>
