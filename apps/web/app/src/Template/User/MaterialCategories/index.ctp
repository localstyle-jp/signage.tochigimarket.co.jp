...
<div class="content_inr">
  ...
  <div class="box">
    ...
    <div class="btn_area" style="margin-top:10px;"><a
        href="<?= $this->Url->build(array('action' => 'edit', '?' => ['parent_id' => $query['parent_id']])); ?>"
        class="btn btn-primary w-20 rounded-pill"><i class="far fa-plus-square"></i> 新規登録</a></div>

    <div class="btn_area" style="margin-top:10px; text-align:right; margin-right:30px;">
      <a href="<?= $this->Url->build(array('controller' => 'materials', 'action' => 'index')); ?>"
        class="btn btn-primary">素材一覧</a>
    </div>
      ...
      <nav aria-label="breadcrumb">
        <ul class="breadcrumb">
          <li class="breadcrumb-item"><a
              href="<?= $this->Url->build(['action' => 'index', '?' => []]); ?>">ルート</a>
          </li>
          <?php $parent_categories = $parent_category; ?>
          <?php while ($pcat = array_pop($parent_categories)): ?>
          <?php if ($query['parent_id'] == $pcat->id): ?>
          <li class="breadcrumb-item active"><?= h($pcat->name); ?>
          </li>
          <?php else: ?>
          <li class="breadcrumb-item"><a
              href="<?= $this->Url->build(['action' => 'index', '?' => ['parent_id' => $pcat->id]]); ?>"><?= h($pcat->name); ?></a>
          </li>
          <?php endif; ?>
          <?php endwhile; ?>
        </ul>
        ...
        <tr
          ...
          <td>
            <div class="btn_area" style="text-align:left;">
              <a href="<?= $this->Url->build(['action' => 'index', '?' => ['parent_id' => $data->id]]); ?>"
                class="btn btn-primary btn-sm"><i class="fas fa-edit"></i> 下層カテゴリ</a>
            </div>
          </td>
