...
<body>
  ...
  <script>
    ...
    function checkOnline() {
      fetch(
          "<?= $this->Url->build('/phpinfo.php', true); ?>")
        .then(res => {
          $('.offline_alert_container').remove();
        })
        .catch(error => {
          $('body').append(
            '<div class="offline_alert_container" style="position: fixed; top: 0px; left: 0px;"><p style="font-size: 30px;">サーバに接続できません。</p></div>'
          );
          var now = new Date();
    ...
    var scene_manager = function() {
      ...
      var show = function() {
        ...
        $.each(mp4, function(i, val) {
          if (items[scene_list[index]].action == 'play_mp4_' + val.no || items[scene_list[index]].action ==
            'play_page_mp4_' + val.no) {
            playMp4(i, 0);
            return false;
          }
        });
