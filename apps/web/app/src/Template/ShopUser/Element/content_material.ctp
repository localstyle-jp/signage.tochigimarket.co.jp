...
<tr id="block_no_<?= $rownum; ?>" class="first-dir item_block">
  <!-- カテゴリ / 素材名 -->
  <td class="show_sp">
    <div>#<?= ($rownum + 1); ?> <?= h($material['material']['name']) ?></div>
  </td>
  ...
  <td class="text-center">
    ...
    <?php if($material['material']['type'] == Material::TYPE_IMAGE):?>
      <div>
        <img src="<?= h($material['material']['attaches']['image']['s']) ?>" class="w-100">
      </div>
    ...
    <?php elseif ($material['material']['type'] == Material::TYPE_MOVIE_MP4): ?>
      <video playsinline controls muted width="300px;" id="mate_mp4_<?= $material['id'] ?>">
        <source src="<?= $material['material']['attaches']['file']['src']; ?>#t=1,2">
      </video>
    ...
    <?php if (!$locked): ?>
      ...
      <div class="btn-group">
        ...
        <div class="dropdown-menu" style="width: 200px;">
          <button type="button" class="dropdown-item btn btn-light up" onclick="clickSort(<?= h($rownum); ?>, 'first');"><i class="fas fa-angle-double-up"></i> 最上段に移動</button>
          <button type="button" class="dropdown-item btn btn-light up" onclick="clickSort(<?= h($rownum); ?>, 'up');"><i class="fas fa-angle-up"></i> １つ上に移動</button>
          <div class="dropdown-divider up down"></div>
          <button type="button" class="dropdown-item btn btn-light down" onclick="clickSort(<?= h($rownum); ?>, 'down');"><i class="fas fa-angle-down"></i> １つ下に移動</button>
          <button type="button" class="dropdown-item btn btn-light down" onclick="clickSort(<?= h($rownum); ?>, 'last');"><i class="fas fa-angle-double-down"></i> 最下段に移動</button>
          <div class="dropdown-divider"></div>
          <div class="text-center">
            <button type="button" class="btn btn-sm btn-secondary delete_row" data-row="<?= h($rownum); ?>"><i class="far fa-trash-alt"></i> 削除</button>
          </div>
        </div>
      </div>
    <?php endif; ?>
