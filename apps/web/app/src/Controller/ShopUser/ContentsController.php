...
class ContentsController extends AppController {
    ...
    protected function _getConditions($query) {
        ...
        $ids = [0, $user_id];
        $cond[$cnt++]['Contents.user_id in'] = $ids;

        $admin_ids = $this->Users->getAdminIds();
        $admin_ids[] = 0;
        if ($query['sch_user_type'] == 'admin') {
        } elseif ($query['sch_user_type'] == 'shop') {
            $admin_ids = [$this->Session->read('userid')];
        } else {
            $admin_ids[] = $this->Session->read('userid');
        }
        $cond[$cnt++]['Contents.user_id in'] = $admin_ids;
    ...
    public function edit($id = 0) {
        ...
        $callback = function ($id) use ($delete_ids) {
            ...
            $this->Flash->set('「最新版にする」をタップしてください');
        };
    ...
    public function setList() {
        ...
        $sound_list = [];
        $sound_list = $this->Materials->find('list', ['keyField' => 'file', 'valueField' => 'name'])->where(['Materials.type' => Material::TYPE_SOUND])->order(['Materials.position' => 'ASC'])->toArray();
        $list['sound_list'] = $sound_list;
